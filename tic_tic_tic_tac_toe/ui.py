...
class Board(GridLayout):
    ...
    def handle_move(self, row, col):
        ...
        if (
            self.game.current_turn is not None
            and self.game.current_turn != self.players[current].symbol
        ):
    ...
    def update_lines(self, *args):
        """
        Updates the position and size of grid lines when the
        board is resized or moved.
        """
        w, h = self.size
        x, y = self.pos
        cell_w = w / 3
